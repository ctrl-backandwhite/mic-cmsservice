 ...
 *
 * Responsabilidades:
 * 1. Calcular puntos por compra: floor(totalUSD × pointsPerUnit ×
 * tierMultiplier)
 * 2. Resolver el tier del usuario según su balance de puntos
 * 3. Detectar cambios de nivel (level-up)
 * 4. Otorgar puntos bonus al subir de nivel
 * 5. Garantizar idempotencia (no duplicar puntos por la misma orden)
 * 6. Publicar eventos de loyalty a Kafka
 */
@Log4j2
@Service
...
public class LoyaltyMachine {
     ...
     * @param totalAmountUsd Monto total de la orden en USD
     * @param orderId        ID de la orden (para idempotencia y trazabilidad)
    ...
    public LoyaltyResult processOrderDelivery(String userId, BigDecimal totalAmountUsd, String orderId) {
        ...
        if (orderId != null && loyaltyRepository.existsEarnTransactionByOrderId(orderId)) {
            ...
            return LoyaltyResult.builder()
                    ...
                    .currentTier(currentTier != null ? currentTier.getName() : "Bronze")
                    .leveledUp(false)
                    .build();
        }
        ...
        if (earnedPoints <= 0) {
            log.debug("::> No loyalty points for order={} (amount={}, ppUnit={}, mult={})",
                    orderId, totalAmountUsd, pointsPerUnit, multiplier);
        ...
        loyaltyRepository.saveTransaction(LoyaltyTransaction.builder()
                ...
                .description("Purchase reward: " + earnedPoints + " pts"
                        + " (x" + multiplier.stripTrailingZeros().toPlainString() + " "
        ...
        // ── 6. Detectar level-up ──
        boolean leveledUp = tierAfter != null
                && (tierBefore == null || !tierAfter.getId().equals(tierBefore.getId()));
        int bonusPoints = 0;
        ...
        if (leveledUp) {
            ...
            log.info("::> 🎉 User {} leveled up: {} → {} (+{} bonus pts)",
                    userId,
                    tierBefore != null ? tierBefore.getName() : "None",
                    tierAfter.getName(),
                    bonusPoints);
        }
        ...
        log.info("::> Loyalty processed: user={}, earned={}, bonus={}, total={}, tier={}, levelUp={}",
                userId, earnedPoints, bonusPoints, balanceAfter, tierName, leveledUp);
    ...
    /**
     * Resuelve el tier de un usuario dado su balance de puntos.
     * Itera los tiers ordenados por minPoints ASC; el último que matchea gana.
     *
     * @param points Balance de puntos acumulados
