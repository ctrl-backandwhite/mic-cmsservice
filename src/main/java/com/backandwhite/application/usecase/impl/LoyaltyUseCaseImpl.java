...
public class LoyaltyUseCaseImpl implements LoyaltyUseCase {
    ...
    public LoyaltyTransaction earnPoints(String userId, int points, String description, String orderId) {
        LoyaltyTransaction tx = loyaltyRepository.saveTransaction(
                LoyaltyTransaction.builder().userId(userId).points(points).type(LoyaltyTransactionType.EARN)
                        .description(description).orderId(orderId).createdAt(Instant.now()).build());
        int totalPoints = loyaltyRepository.sumPointsByUserId(userId);
        cmsEventPort.publishLoyaltyPointsEarned(userId, orderId, points, totalPoints, null);
        return tx;
    }
    ...
    public LoyaltyTransaction redeemPoints(String userId, int points, String description, String orderId) {
        ...
        LoyaltyTransaction tx = loyaltyRepository.saveTransaction(
                LoyaltyTransaction.builder().userId(userId).points(-points).type(LoyaltyTransactionType.REDEEM)
                        .description(description).orderId(orderId).createdAt(Instant.now()).build());
        int totalPoints = loyaltyRepository.sumPointsByUserId(userId);
        cmsEventPort.publishLoyaltyPointsRedeemed(userId, points, totalPoints, null, null);
        return tx;
    }
