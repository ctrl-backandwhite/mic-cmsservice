...
public class SeoPageUseCaseImpl implements SeoPageUseCase {
    ...
    @Override
    @Transactional(readOnly = true)
    public PaginationDtoOut<SeoPage> findAll(int page, int size, String sortBy, boolean ascending) {
        var pageable = PageableUtils.toPageable(page, size, sortBy, ascending);
        return PageableUtils.toResponse(seoPageRepository.findAll(pageable));
    }
